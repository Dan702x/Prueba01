...
import {
  ...
  ArrowPathIcon // <-- CAMBIO: Importado para el botón Limpiar
} from '@heroicons/react/24/solid';
import ModalBase from '../../components/common/ModalBase';
import FormularioParticipante from './FormularioParticipante'; // Nombre actualizado
...
export default function GestionParticipantes() {
  ...
  const [areaFilter, setAreaFilter] = useState('Todos'); // <-- CAMBIO: Nombre de estado
  // --- CAMBIO: Estados de fecha eliminados ---
  ...
  const participantesFiltrados = useMemo(() => {
    ...
    return participantes.filter(p => {
      ...
      // --- CAMBIO: Lógica de filtro de fecha eliminada ---
      
      return searchMatch && areaMatch;
    });
  }, [participantes, searchTerm, areaFilter]); // <-- CAMBIO: Dependencias actualizadas
  ...
  const handleClearFilters = () => {
    setSearchTerm('');
    setAreaFilter('Todos');
    // --- CAMBIO: Setters de fecha eliminados ---
    setCurrentPage(1);
  };
  ...
  const handleToggleActivo = () => { setParticipantes(participantes.map(p => p.id === modal.data.id ? { ...p, activo: !p.activo } : p)); closeModal(); };
  ...
  const handleDownloadTemplate = async () => {
    ...
    // --- CAMBIO 1: Ajustado a 5 columnas (A hasta E) ---
    ...
    // --- CAMBIO 2: Headers actualizados (5 columnas) ---
    ...
    // --- CAMBIO 3: Fila de ejemplo actualizada (5 columnas) ---
  ...
  const handleExport = async () => {
    ...
    ws.mergeCells('A1:G1'); // 7 columnas
      ...
      <div className="bg-white p-6 rounded-xl shadow-md border border-gray-200">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-4 items-end">
          ...
          <div className="col-span-1 sm:col-span-1 lg:col-span-4">
            <label htmlFor="area-proyecto" className="block text-sm font-medium text-gray-700 mb-1">Área</label>
...
<div className="col-span-1 sm:col-span-1 lg:col-span-2">
             <button 
                ...
                className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-blue-600 text-white font-semibold rounded-lg shadow-sm hover:bg-blue-700" // <-- CLASES ACTUALIZADAS
              >
      ...
      <ModalBase isOpen={modal.isOpen} onClose={closeModal} maxWidth="md:max-w-xl">
        { (modal.type === 'new' || modal.type === 'edit') && <FormularioParticipante onClose={closeModal} onSave={handleSave} practicante={modal.data} /> }
