  ...
  const handleSaveEvento = useCallback((datosFormulario) => {
    setEventos(prevEventos => {
      if (eventoSeleccionado) {
        return prevEventos.map(e => e.id === eventoSeleccionado.id ? { ...e, ...datosFormulario, duracion: datosFormulario.duracion } : e);
      } else {
        const nuevoEvento = { id: Date.now(), ...datosFormulario, duracion: datosFormulario.duracion, estado: true };
          ...
          <table className="w-full text-sm text-left text-gray-600">
            <thead className="text-xs text-gray-700 uppercase bg-gray-50 sticky top-0 z-10">
              <tr>
                ...
                <th scope="col" className="px-6 py-4">Duración</th> {/* <-- NUEVA COLUMNA */}
              ...
              {currentItems.length > 0 ? (
                currentItems.map((evento) => (
                  <tr key={evento.id} className={`bg-white border-b hover:bg-gray-50 ${!evento.estado ? 'bg-gray-50' : ''}`}>
                    ...
                    <td className={`px-6 py-4 ${!evento.estado ? 'text-gray-400' : ''}`}>{evento.duracion || 'N/A'}</td> {/* <-- NUEVA CELDA */}
