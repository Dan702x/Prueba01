...
export default function BarraLateralSuper({ items = [], helpItem, logoSrc, isCollapsed, onToggleSidebar }) {
  ...
  const getLinkClasses = ({ isActive }) => {
    ...
    if (isActive) {
      ...
      // --- CAMBIO 1: Cambiado de border-l-[20px] a border-l-[15px] ---
          ...
          {items.map((item, index) => (
            <li key={index} className="mb-2">
              <NavLink to={item.to} className={getLinkClasses}>
                {({ isActive }) => (
                  <>
                    {/* --- CAMBIO 2: Cambiado de -ml-[20px] a -ml-[15px] --- */}
                    ...
                    <span 
                      className={`ml-3 whitespace-nowrap ${isCollapsed ? 'hidden' : 'block'}`}
                    >
                      {item.label}
                    </span>
                  </>
      ...
      {helpItem && (
        <div className="mt-auto pt-4 border-t border-[#304070] mb-3">
          <NavLink to={helpItem.to} className={getLinkClasses}>
            {({ isActive }) => (
              <>
                {/* --- CAMBIO 3: Cambiado de -ml-[20px] a -ml-[15px] --- */}
                <div className={`${isCollapsed ? '-ml-[15px]' : ''}`}>
